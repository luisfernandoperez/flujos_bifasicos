...
def Parametros(d, rho_l, w_l, mu_l, rho_g, w_g, mu_g, T, theta=0, g=9.81):
    ...
    *---------------------------'''
    ...
    Beggs_Brills(Vs_g, Vs_l, rho_l, rho_g, mu_l, mu_g, d, g, theta, T)


def Beggs_Brills(Vs_g, Vs_l, rho_l, rho_g, mu_l, mu_g, d, g, theta, T):
    ...
    #Fracción volumétrica superficial
    lambda_l = Vs_l/V_m
    ...
    #print(F_r, patron,H_l )
    grad_fricc = grad_fricc_beggs(V_m, d, lambda_l, H_l, rho_l, rho_g, mu_l, mu_g, e, rho_m)
    
    #Densidad de deslizamiento
    ...
    #p  = rho_g*T*R/M #solo el fas
    
    #Si se usan los datos de la mezcla en vez de solo el gas
    M_m = M*(1 - H_l) + 18.01528/1000*H_l # promedio de la mezcla con la masa molar del agua
    p  = rho_m*T*R/M
    ...
    print('* El flujo está en patrón ' + patron)
    


def grad_fricc_beggs(V_m, d, lambda_l, H_l, rho_l, rho_g, mu_l, mu_g, e, rho_m):
    
    
    mu_m = mu_l*lambda_l + mu_g*(1. - lambda_l)
    
    Re_n = rho_m*V_m*d/mu_m
...
def Patrones_Beggs_Brills(lambda_l, F_r):
    ...
    respuesta = ''
    #print(Segregado, Transicion, Intermitente, Distribuido)
...
def Dukler_Eaton(Vs_g, Vs_l, rho_l, rho_g, mu_l, mu_g, d, g):
    ...
    #Valor del articulo de dukler
    f_n = 0.00140 + 0.125*power(Re_k,-0.32)
    
    #Valor dado en la clase por el profe
    #f_n = 0.0056 + 0.5*power(Re_k,0.32)
...
def Lockhart_Martinelli(d, rho_l, Vs_l, mu_l, rho_g, Vs_g, mu_g, w_l, w_g, x, A):
    ...
    grad_presion_superficial = 2.*f_l*(Vs_l*A*rho_l)**2/(d*A**2*rho_l)
    
    grad_presion = phi_l*grad_presion_superficial
    print(grad_presion_superficial)
